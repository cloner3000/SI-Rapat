			...
			<div class="detail_event">
				<?php 
					if(isset($events)){
						...
						foreach($events as $e){
						 if($i % 2 == 0){
								echo '<div class="info2"><h4>'.$e['time'].'<img src="'.base_url().'assets/css/images/edit.png" alt="" title="|id:'.$e['id_user'].'|no:'.$e['id'].'" /></h4><p>'.$e['event'].'</p></div>';
							}else{
								echo '<div class="info2"><h4>'.$e['time'].'<img src="'.base_url().'assets/css/images/edit.png" alt="" title="|id:'.$e['id_user'].'|no:'.$e['id'].'" /></h4><p>'.$e['event'].'</p></div>';
							} 
							$i++;
...
<script type="text/javascript">
        $(".detail").live('click',function(){
            ...
            $.ajax({
                type: 'post',
                dataType: 'json',
                url: "<?php echo site_url("kajur/evencal/detail_event");?>",
                ...
                success: function( data ) {
                    ...
                    if(data.status){
                        ...
                        $.each(data.data, function(index, value) {
                            if(i % 2 == 0){
                                html = html+'<div class="info2"><h4>'+value.time+'<img src="<?php echo base_url();?>assets/css/images/edit.png" alt="" title="|id:'+value.id_user+'|no:'+value.id+'" /></h4><p>'+value.event+'</p></div>';
                            }else{
                                html = html+'<div class="info2"><h4>'+value.time+'<img src="<?php echo base_url();?>assets/css/images/edit.png" alt="" title="|id:'+value.id_user+'|no:'+value.id+'" /></h4><p>'+value.event+'</p></div>';
                            } 
                            i++;
        ...
        $(".delete").live("click", function() {
            if(confirm('Are you sure delete this event ?')){
                ...
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: "<?php echo site_url("kajur/evencal/delete_event");?>",
        ...
        $(".add_event").live('click', function(){
            $.colorbox({ 
                    overlayClose: false,
                    href: '<?php echo site_url('kajur/evencal/add_event');?>',
