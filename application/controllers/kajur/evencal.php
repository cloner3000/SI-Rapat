...
class Evencal extends CI_Controller {
	...
	function add_event(){
		...
		$this->load->view('kajur/calendar/add_event', $data);
	}
	...
	function do_add(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$this->evencal->addEvent($this->input->post('year', true), 
										...
										$this->input->post('id_user', true)
										);
			echo json_encode(array('status' => true, 'time' => $this->input->post('time', true),  'event' => $this->input->post('event', true), 'id_user'=>$this->input->post('id_user', true)));
		}
	}
	...
	function _setting(){
		return array(
			'start_day' 		=> 'monday',
			'show_next_prev' 	=> true,
			'next_prev_url' 	=> site_url('kajur/evencal/index'),
			'month_type'   		=> 'long',
            'day_type'     		=> 'short',
