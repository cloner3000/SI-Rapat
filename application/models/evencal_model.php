...
class Evencal_model extends CI_Model {
	...
	function getEvent($year, $month, $day){
		...
		$query = $this->db->select('idevent as id, event_start as time, event, id_user')->order_by('event_start')->get_where('event_detail', array('event_date' => $year));
		if($query->num_rows() > 0){
	...
	// insert event
	function addEvent($year, $month, $day, $time, $event, $id_user){	
		...
		if($check->num_rows() > 0){
			...
			$this->db->insert('event_detail', array('event_date' => "$year-$month-$day", 'event_start' => $time, 'event' => $event, 'id_user'=>$id_user));
		}else{
